...
impl WakerImpl {
    pub fn new(id: TWakerID, resume: Box<dyn Fn(TWakerID)>) -> Self {
        Self { id, resume }
    }
}
...
pub fn make(id: TWakerID, resume: Box<dyn Fn(TWakerID)>) -> Waker {
    let waker = Arc::new(WakerImpl::new(id, resume));
